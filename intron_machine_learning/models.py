...
class MFNetClassifierWithStart(nn.Module):
    ...
    def __init__(self, num_classes):
        ...
        self.ff = FeedForward(dim=512*2+2, hidden_dim=1024*2, dropout=0.1)        #+2 if also include the length
        self.head = nn.Linear(512*2+2, num_classes) # +2 if also include the length # 1024 if 500m model

    def forward(self, sequences, starts, length):#, lengths):
