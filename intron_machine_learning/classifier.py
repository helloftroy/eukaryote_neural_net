...
import wandb
import numpy as np
from models import MFNetRegressor, MFNetClassifier
...
config = {
    ...
    "grad_acc": 16,
    "class_weighting": "10",
    "other": "using the introns with their abundance"
...
model = MFNetClassifier(num_classes=config["classes"]).cuda()
#print(model)
...
def evaluate():
    ...
    wandb.log({"test_loss": avg_loss, "test_accuracy": accuracy, "f1_score": f1,
            "pr": wandb.plot.line(table, "recall_micro", "precision_micro", stroke=None, title="Average Precision"),
